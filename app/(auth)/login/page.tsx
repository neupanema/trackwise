...
function detectInAppBrowser(): boolean {
  ...
  const inAppPattern = /FBAN|FBAV|FB_IAB|FB4A|FBIOS|Instagram|GSA|MicroMessenger|Twitter|Line\/|wv/i;
  const iosWebView = /iPhone|iPad|iPod/.test(ua) && !/Safari/.test(ua) && /AppleWebKit/.test(ua);
  return inAppPattern.test(ua) || iosWebView;
}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-indigo-300 mb-1">
                    Google sign-in not available here
                  </p>
                  <p className="text-xs leading-relaxed" style={{ color: "#9ca3af" }}>
                    Google sign-in doesn't work inside Instagram or Facebook. Open this page in Chrome or Safari to use Google sign-in. You can still sign in with your email and password above.
                  </p>
                  <button
                    type="button"
                    onClick={openInBrowser}
                    className="mt-3 w-full py-2.5 rounded-lg text-sm font-medium text-white flex items-center justify-center gap-2 transition-opacity hover:opacity-90"
                    style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)" }}>
